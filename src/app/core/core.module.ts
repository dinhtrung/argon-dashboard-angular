...
export function translatePartialLoader(http: HttpClient): TranslateLoader {
  return new MultiTranslateHttpLoader(http, [
    { prefix: 'i18n/', suffix: `.json?buildTimestamp=${BUILD_TIMESTAMP}` },
    // + static files overload
    { prefix: 'assets/i18n/', suffix: `.json?buildTimestamp=${BUILD_TIMESTAMP}` },
    // + server side overload
    { prefix: '/api/public/static/', suffix: `.json?buildTimestamp=${BUILD_TIMESTAMP}` }
  ]);
}
