...
export class ButtonTypeComponent extends FieldType {
  ...
  processResponse(res: any): void {
    ...
    if (this.to.patch) {
      if (_.isString(this.to.patch)) {
        this.form.patchValue(JSON.parse(_.template(this.to.patch)(_.assign({}, this.model, this.to.placeholders))));
      } else {
        this.form.patchValue(this.to.patch);
      }
    }
